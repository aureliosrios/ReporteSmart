...
# ---------------------------------------------------------
# 1. CATALOGOS Y LÍNEA BASE DEL PROYECTO (CRONOGRAMA DIARIO PV)
# ---------------------------------------------------------

metadata = {
    "proyecto": "Redes Sanitarias de Agua Potable y Alcantarillado - Habilitación Urbana Los Cedros",
    "codigo_proyecto": "HU-CEDROS-2026",
    "version_esquema": "5.0.0",
    "moneda": "PEN",
    "descripcion": "Base de datos viva oficial para Google Sheets. Incluye Cronograma Valorizado DIARIO (PV por día y WBS). El campo solo envía CANTIDAD. Las fórmulas de Sheets calculan AC, EV, CPI y SPI en tiempo real cruzados con el PV diario a la fecha."
}
...
# 1.4 CRONOGRAMA VALORIZADO DIARIO DE AVANCE PROGRAMADO (PV DIARIO POR WBS)
cronograma_pv_diario = [
    # DÍA 1 (2026-08-01)
    {"fecha": "2026-08-01", "codigo_wbs": "WBS-100", "pv_diario_pen": 5824.10, "pv_acumulado_pen": 5824.10},
    {"fecha": "2026-08-01", "codigo_wbs": "WBS-200", "pv_diario_pen": 3500.00, "pv_acumulado_pen": 3500.00},
    {"fecha": "2026-08-01", "codigo_wbs": "WBS-300", "pv_diario_pen": 0.00, "pv_acumulado_pen": 0.00},
    {"fecha": "2026-08-01", "codigo_wbs": "WBS-400", "pv_diario_pen": 0.00, "pv_acumulado_pen": 0.00},

    # DÍA 2 (2026-08-02)
    {"fecha": "2026-08-02", "codigo_wbs": "WBS-100", "pv_diario_pen": 2472.30, "pv_acumulado_pen": 8296.40},
    {"fecha": "2026-08-02", "codigo_wbs": "WBS-200", "pv_diario_pen": 8500.00, "pv_acumulado_pen": 12000.00},
    {"fecha": "2026-08-02", "codigo_wbs": "WBS-300", "pv_diario_pen": 0.00, "pv_acumulado_pen": 0.00},
    {"fecha": "2026-08-02", "codigo_wbs": "WBS-400", "pv_diario_pen": 0.00, "pv_acumulado_pen": 0.00},

    # DÍA 3 (2026-08-03)
    {"fecha": "2026-08-03", "codigo_wbs": "WBS-100", "pv_diario_pen": 0.00, "pv_acumulado_pen": 8296.40},
    {"fecha": "2026-08-03", "codigo_wbs": "WBS-200", "pv_diario_pen": 19500.00, "pv_acumulado_pen": 31500.00},
    {"fecha": "2026-08-03", "codigo_wbs": "WBS-300", "pv_diario_pen": 4000.00, "pv_acumulado_pen": 4000.00},
    {"fecha": "2026-08-03", "codigo_wbs": "WBS-400", "pv_diario_pen": 0.00, "pv_acumulado_pen": 0.00},

    # DÍA 4 (2026-08-04)
    {"fecha": "2026-08-04", "codigo_wbs": "WBS-100", "pv_diario_pen": 0.00, "pv_acumulado_pen": 8296.40},
    {"fecha": "2026-08-04", "codigo_wbs": "WBS-200", "pv_diario_pen": 18000.00, "pv_acumulado_pen": 49500.00},
    {"fecha": "2026-08-04", "codigo_wbs": "WBS-300", "pv_diario_pen": 7000.00, "pv_acumulado_pen": 11000.00},
    {"fecha": "2026-08-04", "codigo_wbs": "WBS-400", "pv_diario_pen": 0.00, "pv_acumulado_pen": 0.00},

    # DÍA 5 (2026-08-05)
    {"fecha": "2026-08-05", "codigo_wbs": "WBS-100", "pv_diario_pen": 0.00, "pv_acumulado_pen": 8296.40},
    {"fecha": "2026-08-05", "codigo_wbs": "WBS-200", "pv_diario_pen": 12000.00, "pv_acumulado_pen": 61500.00},
    {"fecha": "2026-08-05", "codigo_wbs": "WBS-300", "pv_diario_pen": 5000.00, "pv_acumulado_pen": 16000.00},
    {"fecha": "2026-08-05", "codigo_wbs": "WBS-400", "pv_diario_pen": 2500.00, "pv_acumulado_pen": 2500.00}
]

# ---------------------------------------------------------
# 2. ENTRADAS DE CAMPO (EL CAMPO SOLO ENVÍA CANTIDAD)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 3. GENERACIÓN DEL JSON CON CRONOGRAMA DIARIO PV
# ---------------------------------------------------------
...
db_sheets_json = {
    ...
    "linea_base_proyecto": {
        ...
        "cronograma_pv_diario": cronograma_pv_diario
    },
    ...
    "registros_diarios_campo": field_entries_raw
}
...
print(f"[OK] Archivo JSON para Google Sheets creado en: {json_sheets_path}")


# ---------------------------------------------------------
# 4. EXPORTACIÓN A EXCEL CON FÓRMULAS VIVAS DE PV DIARIO
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# PESTAÑA 1: RESULTADO_OPERATIVO (DASHBOARD CON CRONOGRAMA PV DIARIO)
# ---------------------------------------------------------
ws_ro = wb.create_sheet(title="RESULTADO_OPERATIVO")
ws_ro.views.sheetView[0].showGridLines = True

ws_ro.merge_cells("A1:I2")
ws_ro["A1"] = "RESULTADO OPERATIVO (RO) Y CONTROL EVM - CRONOGRAMA PV DIARIO EN GOOGLE SHEETS"
ws_ro["A1"].font = font_title
ws_ro["A1"].fill = fill_dark
ws_ro["A1"].alignment = Alignment(horizontal="center", vertical="center")

ws_ro["A3"] = "Habilitación Urbana Los Cedros | Cruce de PV Cronograma Diario vs AC/EV Reportado por Celulares"
ws_ro["A3"].font = font_section

# KPIs Fila 5-6
kpi_titles = ["Presupuesto BAC", "Planificado Acum (PV)", "Valor Ganado Exec (EV)", "Costo Real Incurrido (AC)", "Variación Costo (CV)", "CPI Costo", "SPI Plazo (EV/PV)", "EAC Proyectado", "Desvío (EAC - BAC)"]
kpi_forms = [
    "=SUM(CRONOGRAMA_PV_DIARIO!F2:F5)",
    "=SUM(CRONOGRAMA_PV_DIARIO!C2:C21)",
...
# SECCIÓN 2: COMPARATIVA DIARIA PV vs EV vs AC (Días 1 al 5) (Filas 9 a 17)
ws_ro["A9"] = "CRONOGRAMA DIARIO: COMPARATIVA DE PLANIFICADO (PV) VS EJECUTADO (EV) Y COSTO (AC)"
...
# SECCIÓN 3: RESUMEN EVM POR WBS (Filas 19+)
ws_ro["A18"] = "DESEMPEÑO ACUMULADO AL DÍA 5 POR FRENTE DE TRABAJO (WBS)"
ws_ro["A18"].font = font_section

headers_ro_wbs = ["Código WBS", "Nombre Frente WBS", "Presupuesto BAC (S/)", "PV Acum Programado (S/)", "AC Costo Real (S/)", "EV Valor Ganado (S/)", "Diferencia (EV - AC)", "CPI Costo", "SPI Plazo", "EAC Proyectado (S/)"]
...
for r_w, wbs in enumerate(presupuesto_wbs_bac, start=20):
    ...
    # PV Acumulado por WBS desde CRONOGRAMA_PV_DIARIO
    ws_ro.cell(row=r_w, column=4, value=f"=SUMIF(CRONOGRAMA_PV_DIARIO!B:B, A{r_w}, CRONOGRAMA_PV_DIARIO!C:C)").number_format = 'S/ #,##0.00'
...
# ---------------------------------------------------------
# PESTAÑA 2: LOG_FIELD_ENTRIES (REPORTE DIARIO DE CAMPO)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# PESTAÑA 3: CRONOGRAMA_PV_DIARIO (AVANCE PROGRAMADO POR DÍA Y WBS)
# ---------------------------------------------------------
ws_pv_d = wb.create_sheet(title="CRONOGRAMA_PV_DIARIO")
ws_pv_d.views.sheetView[0].showGridLines = True

headers_pv_d = ["Fecha", "Código WBS", "PV Diario Programado (S/)", "PV Acumulado Programado (S/)", "Nombre Frente WBS", "Presupuesto BAC (S/)"]
...
for r_idx, row in enumerate(cronograma_pv_diario, start=2):
    ...
    ws_pv_d.cell(row=r_idx, column=5, value=w_name).font = font_regular
    
    cell_bac = ws_pv_d.cell(row=r_idx, column=6, value=w_bac)
...
print(f"[OK] Archivo Excel de Base de Datos Viva para Google Sheets con Cronograma Diario PV creado en: {excel_sheets_path}")
