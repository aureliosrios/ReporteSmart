...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function mergeLogs(...sources) {
        ...
        sources.flat().map(normalizeLog).filter(Boolean).forEach(log => {
            const key = String(log.id || `${log.fecha}|${log.wbs}|${log.codigoRecurso}|${log.cantidad}|${log.costo}`);
    ...
    async function initApp() {
        ...
        try {
            const resp = await fetch('presupuesto_con_apu.json');
            if (resp.ok) {
        ...
        try {
            const dbResp = await fetch('base_datos_ro_diaria.json');
            if (dbResp.ok) {
                baseDatosData = await dbResp.json();
                dbLogs = mergeLogs(dbLogs, baseDatosData.registros_diarios_logs || []);
    ...
    async function syncLogsToExcelServer(newRecords) {
        ...
        // Usar URL absoluta para garantizar comunicación incluso en file:///
        const apiUrl = 'http://localhost:8080/api/save-log';
    ...
    function setupTareadorPortal() {
        ...
        document.getElementById('btn-save-tareador').onclick = () => {
            ...
            tbody.querySelectorAll('tr').forEach(tr => {
                ...
                if (totalHH > 0) {
                    recordsToSave.push({
                        id: Date.now() + Math.random(), fecha, rol: "Tareador (Bildin)", wbs,
    ...
    function setupAlmaceneroPortal() {
        ...
        document.getElementById('btn-save-almacenero').onclick = () => {
            ...
            tbody.querySelectorAll('tr').forEach(tr => {
                ...
                if (cant > 0) {
                    recordsToSave.push({
                        id: Date.now() + Math.random(), fecha, rol: "Almacenero", wbs,
    ...
    function setupAdminPortal() {
        ...
        document.getElementById('btn-save-admin').onclick = () => {
            ...
            tbody.querySelectorAll('tr').forEach(tr => {
                ...
                if (cant > 0) {
                    recordsToSave.push({
                        id: Date.now() + Math.random(), fecha, rol: "Administradora", wbs,
                        codigoRecurso, tipo: "AC_EQP", detalle: desc, cantidad: cant, unidad, pu: precio, costo
                    });
                }
            });
    ...
    function setupCampoPortal() {
        ...
        document.getElementById('btn-save-campo').onclick = () => {
            ...
            tbody.querySelectorAll('tr').forEach(tr => {
                ...
                if (metrado > 0) {
                    recordsToSave.push({
                        id: Date.now() + Math.random(), fecha, rol: "Ing. de Campo", wbs,
